...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">菜品信息管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active"><span>菜品信息列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">菜品信息列表</h3>							
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									...
									<select name="caixi" id="caixiSelect" class="form-control form-control-sm" aria-controls="tableId">
									</select>
									<label>
										美食名称
									</label>
									<input type="text" id="meishimingchengSearch" class="form-control form-control-sm" placeholder="" aria-controls="tableId">
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('shangjiabianhao')">商家编号<i id="shangjiabianhaoIcon" class="fa fa-sort"></i></th>
											...
											<th>图片</th>
											<th onclick="sort('danjia')">单价<i id="danjiaIcon" class="fa fa-sort"></i></th>

											<th>主料</th>
											<th onclick="sort('fenliang')">份量<i id="fenliangIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function search(){
			...
			var caixiIndex = document.getElementById("caixiSelect").selectedIndex;
			var caixiOptions = document.getElementById("caixiSelect").options;
			var caixiSelectedOption = caixiOptions[caixiIndex]
			if(caixiSelectedOption.text != null && caixiSelectedOption.text != ''){
				searchForm.caixi = "%" + caixiSelectedOption.text + "%";
			}
        ...
        function getDataList() {
			http("caipinxinxi/page","GET",{
				page: pageIndex,
				limit: pageSize,
				sort: sortColumn,
				order: sortOrder,
				shangjiaming : searchForm.shangjiaming,	
				caixi : searchForm.caixi,	
		...
		function setDataRow(item,number){
			...
			var shangjiabianhaoCell  = document.createElement('td');

						 shangjiabianhaoCell.innerHTML = item.shangjiabianhao;


			row.appendChild(shangjiabianhaoCell);
			...
			var zhuliaoCell  = document.createElement('td');

						  if(item.zhuliao != null && item.zhuliao != "" && item.zhuliao.length >= 11){
			  	zhuliaoCell.innerHTML = item.zhuliao.substring(0, 10) + "..."; 
			  }else{
			  	zhuliaoCell.innerHTML = item.zhuliao;
			  }


			row.appendChild(zhuliaoCell);
			...
			var meishijieshaoCell  = document.createElement('td');
						  ...
						  if(item.meishijieshao != null && item.meishijieshao != "" && item.meishijieshao.length >= 11){
			  	meishijieshaoCell.innerHTML = item.meishijieshao.substring(0, 10) + "..."; 
			  }else{
			  	meishijieshaoCell.innerHTML = item.meishijieshao;
			  }


			row.appendChild(meishijieshaoCell);
			...
			crossBtn1.setAttribute("onclick","crossTable(" + item.id + ",\"waimaidingdan\")");
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("caipinxinxi/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
		...
		function crossTable(id,crossTableName){
			...
			window.sessionStorage.setItem('crossTableName',"caipinxinxi");
